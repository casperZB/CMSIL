...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if self.stride > 1:
            ...
            out_mask = F.interpolate(mask.to(x.dtype), size=out_conv.size(-1), mode="nearest")
        else:
...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        if n_qx_stride >= 1:
            ...
            self.query_conv = MaskedConv1D(self.n_embd, self.n_embd, kernel_size, stride=stride, padding=padding, groups=self.n_embd, bias=False)

            # key, value conv (depthwise)
            kernel_size = n_kv_stride + 1 if n_kv_stride > 1 else 3
            stride, padding = n_kv_stride, kernel_size // 2
            self.key_conv = MaskedConv1D(self.n_embd, self.n_embd, kernel_size, stride=stride, padding=padding, groups=self.n_embd, bias=False)
            self.value_conv = MaskedConv1D(self.n_embd, self.n_embd, kernel_size, stride=stride, padding=padding, groups=self.n_embd, bias=False)
        else:
...
class LocalMaskedMHCA(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        diagonal_mask = self._sliding_chunks_query_key_matmul(float_inverse_kv_mask.new_ones(size=float_inverse_kv_mask.size()), float_inverse_kv_mask, 1, self.window_overlap)
        att += diagonal_mask

        # ignore input masking for now
        att = nn.functional.softmax(att, dim=-1)
        # softmax sometimes inserts NaN if all positions are masked, replace them with 0
        att = att.masked_fill(torch.logical_not(kv_mask.squeeze(1)[:, :, None, None]), 0.0)
        att = self.attn_drop(att)
...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = MaskedConv1D(n_embd, width, kernel_size, n_ds_stride, padding=padding)
