...
def parse_args():
    ...
    parser.add_argument("--test_batch_size", default=5, type=int, help="test batch size")
    ...
    parser.add_argument("--learning_rate", default=4.9868745368973294e-5, type=float, help="initial learning rate") # ☆
    ...
    parser.add_argument("--eval_metric", type=str, default="AUC@PR", help="the evalate metric: [AUC@PR, AUC@ROC, ACC]")
    parser.add_argument("--optuna", action="store_true", help="Enable optuna hyperparameter search")
    parser.add_argument("--alpha", default=0.4, type=float, help="alpha for focal loss")
    parser.add_argument("--gamma", default=2.5, type=float, help="gamma for focal loss, if gamma=0, then use BCELoss")
    parser.add_argument("--threshold", default=0.45, type=float, help="threshold for pseudo label")

    # ========================= Model Configs ==========================
    parser.add_argument("--arch", type=lambda s: [int(i) for i in s.split('_')], default="2_1_5", help="[convs, stem transformers, branch transformers]")
    parser.add_argument("--scale_factor", type=int, default=2, help="scale factor between fpn branch layers.")
    parser.add_argument("--max_seq_len", type=int, default=256, help="max sequence length (used for training)") # ☆
    ...
    parser.add_argument("--n_head", type=int, default=8, help="number of head for self-attention in transformers") # ☆
    ...
    parser.add_argument('--pseudo_label', type=str, default='data/XD_frame_pseudo_label.npy', help="the pseudo label path")
